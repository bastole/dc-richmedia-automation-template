;(function(){
	...
	var PreLoader = function(images, options){
		...
		if(options && typeof options == 'object') this.setOptions(options);

		this.addQueue(images);
		if(this.queue.length && this.options.auto) this.processQueue();
	};
	...
	PreLoader.prototype._addEvents = function(image, src, index){
		var self = this,
			...
			abort = function(){
				...
				if(o.onError) o.onError.call(self, src);
				_checkProgress.call(self, src);
				if(o.pipeline) self._loadNext(index);
			},
			load = function(){
				cleanup.call(this);
	...
	PreLoader.prototype._loadNext = function(index){
		...
		index++;
		if(this.queue[index]) this._load(this.queue[index], index);
	...
	function _checkProgress(src, image){
		...
		// call onProgress
		if(o.onProgress && src) o.onProgress.call(this, src, image, this.completed.length);

		if (this.completed.length + this.errors.length === this.queue.length){
			args.push(this.completed);
			if(this.errors.length) args.push(this.errors);
